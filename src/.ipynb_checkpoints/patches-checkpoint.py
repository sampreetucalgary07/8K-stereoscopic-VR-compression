...
def save_sub_image(img,key,value,dest_folder,is_tensor,device):
    """Function to save the sub image."""
    sub_img = get_sub_image(img, value, is_tensor=False, device="cuda").unsqueeze(0)
    save_path = os.path.join(dest_folder, key + "_" + os.path.basename(img))
...
def frame_to_patches(
    ...
    dest_folder = make_folder(
        folder_name=foldername + "_patches",
    ...
    # print(patch_list)
    for img in tqdm(
        imageList, total=len(imageList), desc="Adding patches to dask tasks.."
    ):
        # img_tensor = image_to_tensor(img)
        tasks = []
        for key, value in patch_list.items():
            #print(key, value[0], value[1] - 1, value[2], value[3] - 1)
            tasks.append(save_sub_image(img,key,value,dest_folder,False,device))
...
class filter_patches:
    ...
    def folder(self):
        ...
        dest_folder_path = make_folder(
            folder_name=foldername + "_f_ptchs",
    ...
    def inter_patch_filter(self):
        ...
        for patch_no, patch in tqdm(
            ...
            desc="Filtering patches internally",
        ):
