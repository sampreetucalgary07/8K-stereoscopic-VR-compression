...
from model_blocks import DownBlock, UpBlock
...
class VAE(nn.Module):
    def __init__(self, im_channels, model_config):
        ...
        self.decoder_conv_in = nn.Conv2d(
            self.z_channels, self.down_channels[-1], kernel_size=3, padding=(1, 1)
        )
...
if __name__ == "__main__":
    ...
    model = VAE(3, model_config)
    print(model)
    x = torch.randn(1, 3, 256, 256)
    out, encoder_output = model(x)
    print(out.shape, encoder_output.shape)
